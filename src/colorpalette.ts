...
class ColorPalette {
    ...
    constructor(baseColor: string, baseStep: number = 700) {
        ...
        this.set = {
            label: namedColor,
            baseColor: baseColor,
            baseStep: typeof baseStep !== 'number' ? parseInt(baseStep) : baseStep,
            palette: {},
        };
    }
    ...
    public generateColor(step: number): string {
        ...
        return chroma.hsl(hslColor[0], hslColor[1], hslColor[2]).hex();
    }
     ...
     *
     * @returns {Record<number, string>} - The generated color palette, where each key represents a step and its corresponding value represents the color.
     */
    public generatePalette(): Record<number, string> {
        const steps = [50, 100, 200, 300, 400, 500, 600, 700, 800, 900, 950];
...
export { type ColorSet, ColorPalette };
